...
\hldef{(trap.area} \hlkwb{=} \hldef{(left.area} \hlopt{+} \hldef{right.area)}\hlopt{/}\hlnum{2}\hldef{)}
\end{alltt}
\begin{verbatim}
## [1] 8.6664
\end{verbatim}
...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
  ...
  \hlkwa{if}\hldef{(method} \hlopt{==} \hlsng{"Left"}\hldef{)\{}
    \hlkwd{return} \hldef{(left.area)}
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Right"}\hldef{)\{}
    \hlkwd{return} \hldef{(right.area)}
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Midpoint"}\hldef{)\{}
    \hlkwd{return} \hldef{(mid.area)}
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Trapezoidal"}\hldef{)\{}
    \hlkwd{return} \hldef{(trap.area)}
  \hldef{\}}\hlkwa{else}\hldef{\{}
    \hlkwd{stop}\hldef{(}\hlsng{"Please select a valid method (e.g., 'Left', 'Right', 'Midpoint', 'Trapezoidal')"}\hldef{)}
  \hldef{\}}
  \hlcom{######################################}
...
\hlkwd{riemann.sums}\hldef{(}\hlkwc{fnct} \hldef{= integrand,}
             ...
             \hlkwc{method} \hldef{=} \hlsng{"Left"}\hldef{)}
\end{alltt}
\begin{verbatim}
## [1] 8.7464
\end{verbatim}
